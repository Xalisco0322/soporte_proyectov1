...
methods:{  
    ...
    //Procedimiento CREAR.
    altaEquipo:function(){
        axios.post(url, {opcion:1, cliente:this.cliente,serie:this.serie,imei:this.imei,sim:this.sim,linea:this.linea,alias:this.alias,fecha:this.fecha,plataforma:this.plataforma,nombre:this.nombre,stock:this.stock }).then(response =>{
            this.listarEquipos();
        });        
         this.cliente = "",
         this.serie = "",
    ...
    editarEquipo:function(id,cliente,serie,imei,sim,linea,alias,fecha,plataforma,nombre,stock){       
       axios.post(url, {opcion:2, id:id, cliente:cliente, serie:serie, imei:imei, sim:sim, linea:linea, alias:alias, fecha:fecha, plataforma:plataforma, nombre:nombre, stock:stock }).then(response =>{           
           this.listarEquipos();           
        });                              
    },    
